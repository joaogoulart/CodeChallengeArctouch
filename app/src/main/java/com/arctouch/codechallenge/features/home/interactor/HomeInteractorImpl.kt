...
class HomeInteractorImpl: HomeInteractor {
        ...
        compositeDisposable.add(Observable.concat(fromCache, fromServer)
                ...
                .subscribeBy(onNext = {
                    if (it.isNotEmpty() && !loadedMovies) {
                        getMovies(it, page, defaultLanguage, listener)
                        loadedMovies = true
                        return@subscribeBy
                    }
                }, onError = {
    ...
    private fun getMovies(genres: List<Genre>, page: Long, defaultLanguage: String, listener: HomeInteractor.UpcomingMoviesListener) {
